...
const Search = () => {
    ...
    const { data: people, refetch: peopleRefetch } = useQuery(
        `search_users_${str}`,
    ...
    const { data: questions, refetch: questionsRefetch } = useQuery(
        `search_question_${str}`,
    ...
    const {
        ...
        fetch(`http://localhost:5500/followings/${user?.user_email}`).then(
            (res) => res.json()
        )
    );
        ...
        <>
            ...
            <section className="homePageContainer mx-auto mt-10">
                <h1 className="card p-5">
                    ...
                    people.length === 0 ? (
                        <span>
                            Nothing Found for the result of{' '}
